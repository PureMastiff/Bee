...
class Network():
    ...
    def check(self, instance=None):
        ...
        if exclude_re:
            print("Excluding network devices matching: %s" % exclude_re)
            self._exclude_iface_re = re.compile(exclude_re)
        if Platform.is_linux():
            return self._check_linux(instance)
        elif Platform.is_bsd():
            return self._check_bsd(instance)
    ...
    def _submit_devicemetrics(self, iface, vals_by_metric):
        ...
        for metric, val in vals_by_metric.iteritems():
            ...
            ifaces_net.append(['system.net.{}, {}, {}'.format(metric, int(time.time()), val), {"hostname": "localhost.10.1.2.252", "type": "gauge"}])

            #print metric, val
            #self.rate('system.net.%s' % metric, val, device_name=iface)
            count += 1
            #print("tracked %s network metrics for interface %s" % (count, iface))
        return ifaces_net
    ...
    def _check_linux(self, instance):
        ...
        if self._collect_cx_state:
            try:
                print("Using `ss` to collect connection state")
                ...
                print("`ss` not found: using `netstat` as a fallback")
                ...
                print("Error collecting connection stats.")
        ...
        for l in lines[2:]:
            ...
            if self._parse_value(x[0]) or self._parse_value(x[8]):
                ...
                ret_linux = self._submit_devicemetrics(iface, metrics)
                system_net.extend(ret_linux)
        ...
        try:
            ...
            for key, metric in tcp_metrics_name.iteritems():
                tcp_net.append(['system.net.{}, {}, {}'.
                               format(metric,
                                      int(time.time()),
                                      self._parse_value(tcp_metrics[key])),
                                {"hostname": "localhost.10.1.2.252",
                                 "type": "gauge"}])
            system_net.extend(tcp_net)
            ...
            for key, metric in udp_metrics_name.iteritems():
                if key in udp_metrics:
                    ...
                    udp_net.append(['system.net.{}, {}, {}'.
                                   format(metric,
                                          int(time.time()),
                                          self._parse_value(udp_metrics[key])),
                                    {"hostname": "localhost.10.1.2.252",
                                     "type": "gauge"}])
            system_net.extend(udp_net)
            return system_net
        except IOError:
            print("Unable to read /proc/net/snmp.")
    ...
    def _check_bsd(self, instance):
        ...
        try:
            ...
            for h in ("Ipkts", "Ierrs", "Ibytes", "Opkts", "Oerrs", "Obytes", "Coll"):
                if h not in headers:
                    print("%s not found in %s; cannot parse" % (h, headers))
                    return False
            ...
            for l in lines[1:]:
                ...
                if self._parse_value(x[-5]) or self._parse_value(x[-2]):
                    ...
                    ret = self._submit_devicemetrics(iface, metrics)
                    system_net.extend(ret)
            return system_net
        except SubprocessOutputEmptyError:
            print("Error collecting connection stats.")

        try:
            netstat, _, _ = get_subprocess_output(["netstat", "-s", "-p" "tcp"])
            self._submit_regexed_values(netstat, BSD_TCP_METRICS)
        except SubprocessOutputEmptyError:
            print("Error collecting TCP stats.")
    ...
    def _check_solaris(self, instance):
        try:
            ...
            print("Error collecting kstat stats.")
